'use strict'
const namespace = require('continuation-local-storage').getNamespace('requestNamespace');
...
const request = (...args) => {
  ...
  const addRequestId = (req) => {
      req.headers['x-request-id'] = namespace.get('requestId')
      return req
  }

  service.client.execute({ http: http, operationId: operationId, parameters: parameters, requestInterceptor: addRequestId })
    .then(resolve)
    .catch((err) => {
