...
const getHttpError = (statusCode) => {
  ...
  return require(`./../../../errors/http/${errorName}`)
}
      ...
      response.on('end', () => {
        try {
          ...
          if (parseInt(response.statusCode) >= 400) {
            ...
            const error       = new errorClass(_.get(result, 'message', null), response.statusCode)
            error.setServiceName(request.serviceName)
            error.setOperationId(request.operationId)
            throw error
          }
